...
async function readContract(chainID, metadata, addressSS58)
{
    ...
    var readMessages = [], writeMessages = [];
    let rout = [];
    let wout = [];
...
async function executeRead(n, nm) {
	...
	let inputs = []
	inputs.push(null);
	inputs.push({ gasLimit: -1, storageDepositLimit: null });
	...
	const { gasRequired, storageDeposit, result, output } = await f.apply(null, inputs); 
	if ( result.isOk ) {
...
async function presentWASMContractMessage(m, myapi, contract, n) {
	...
	const { gasRequired, storageDeposit, result, output } = await f("5CMLPVmw5BUrmCsoH5xVDDAFwQz8LVt6vPx16BEsJAePNvVb", { gasLimit: -1, storageDepositLimit });
	if ( result.isOk ) {
