...
function showxcmtransfers(filter) {
    ...
    if (initxcmtransfers) {
        ...
        var table = $(tableName).DataTable({
            ...
            columns: [{
                    ...
                    render: function(data, type, row, meta) {
                        if (type == 'display') {
                            return '<button type="button" class="btn btn-outline-primary text-capitalize">' + data + '</button>';
                        }
                        return data;
                    }
                },
                    ...
                    render: function(data, type, row, meta) {
                        if (type == 'display') {
                            if (row.destAddress !== undefined) {
                                return presentIDRow(row, 'destAddress');
                            } else {
                            ...
                            try {
                                if (row.chainIDDest != undefined && row.chainDestName) {
                                    if (row.incomplete !== undefined && row.incomplete > 0) {
                                        return presentChain(row.idDest, row.chainDestName) + " Incomplete " + presentSuccessFailure(false);
    ...
    let pathParams = `search/xcmtransfers`
    postData(pathParams, filter, tableName);
    setupapidocs("search", "xcmtransfers", filter);
}

var initxcmmessages = false;

function showxcmmessages(filter) {
                ...
                */
                {
                    ...
                    render: function(data, type, row, meta) {
                        if (type == 'display') {
                            if (row.chainName == undefined) {
                                console.log("FAIL", row);
                            }
                            return presentChain(row.id, row.chainName)
                        }
                        return data;
                    }
                },
                {
                    ...
                    render: function(data, type, row, meta) {
                        if (type == 'display') {
                            try {
                                if (row.chainIDDest != undefined && row.chainDestName) {
                                    let s = presentBlockNumber(row.idDest, row.chainDestName, row.blockNumber);
                                    return s;
                                } else {
                                    return "-"
                                }
                ...
                },
                {
                    data: 'msgDecoded',
                    render: function(data, type, row, meta) {
                        return presentXCMMessage(row.msgDecoded, row.msgHash);
                    }
                },
                {
                    data: 'sentAt',
                    render: function(data, type, row, meta) {
                        if (type == 'display') {
                            try {
                                if (row.sentAt !== undefined && row.relayChain != undefined) {
                                    let s = "";
                                    if (row.relayChain.toLowerCase() == 'polkadot') s = presentBlockNumber('polkadot', row.relayChain, row.sentAt);
                                    if (row.relayChain.toLowerCase() == 'kusama') s = presentBlockNumber('kusama', row.relayChain, row.sentAt);
                                    if (s == "") {
                                        console.log("SACOW", row);
                                    }
                                    return s;
                                } else {
                                    return "-"
    ...
    //load data here: warning this function is technically async
    let pathParams = `search/xcmmessages`
    postData(pathParams, filter, tableName);
    setupapidocs("search", "xcmmessages", filter);
}
