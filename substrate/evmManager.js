...
//let debugMethodIDs = []
//let debugTopics = []
let debugMethodIDs = ['0x00000000', '0xeb672419']
...
module.exports = class EvmManager extends PolkaholicDB {
    ...
    decodeLog(log) {
        ...
        let topic0 = topics[0]
        let topicLen = topics.length - 1;
        let lookupID = `${topic0}-${topicLen}`
        let res = {
    ...
    decodeTransactionInput(txn) {
        ...
        if (methodID) {
            ...
            if (r) {
                ...
                try {
                    //var etherjsDecoder = new ethers.utils.Interface([r.abi])
                    var etherjsDecoder = ethTool.initEtherJsDecoder([r.abi])
                    var decoded_raw = etherjsDecoder.decodeFunctionData(methodID, txInput);
                    res.decodeStatus = 'success'
                    res.params = ethTool.processEthersDecodedRaw(r.abi, decoded_raw);
                    return res;
                } catch (err) {
                    res.decodeStatus = "error"
                    res.decodeError = err.toString();
