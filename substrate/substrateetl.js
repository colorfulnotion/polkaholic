...
FROM
  ...
  order by number
    ...
    async pick_chainbalancecrawler(specific_chainID = null, specific_logDT = null) {
        ...
        let sql = `select chainID, UNIX_TIMESTAMP(logDT) as indexTS, jobID, logDT from chainbalancecrawler as c where lastDT < date_sub(Now(), interval 20 SECOND) ${w} order by tally DESC limit 1`;
    ...
    async updateAddressBalances(specific_chainID = null, specific_logDT = null) {
        // pick something your node started already
        let [chainID, indexTS, jobID] = await this.pick_chainbalancecrawler(specific_chainID, specific_logDT);
        ...
        if (chainID == null) {
            ...
            // ( lastUpdateAddressBalancesStartDT < date_sub(Now(), interval 3+POW(3, lastUpdateAddressBalancesAttempts) MINUTE ) or lastUpdateAddressBalancesStartDT is Null  ) and
            let sql = `select chainID, UNIX_TIMESTAMP(logDT) as indexTS from blocklog where chainID in (select chainID from chain where relayChain in ('polkadot','kusama')) and  ( updateAddressBalanceStatus = "Ready" )  and
blocklog.logDT >= '${balanceStartDT}' ${w} order by blocklog.logDT desc, rand()`;
