...
//                            base_fee_per_gas: tx.baseFeePerGas, // this is not real value
    ...
    async update_blocklog(chainID, logDT) {
        ...
        let sqla = {
            ...
            "evmtxs": `select count(*) as numEVMtxs, sum(if(transaction_type = 2, 1, 0)) num1559txs, sum(if(receipt_contract_address is not null, 1, 0)) numEvmContractCreated, sum(if(transaction_type = 0, 1, 0)) numLegacytxs, avg(gas_price / 1000000000) as gasPrice, sum(if(max_fee_per_gas is not null, max_fee_per_gas/ 1000000000, 0)) / sum(if(max_fee_per_gas is not null, 1, 0)) maxFeePerGas, sum(if(max_priority_fee_per_gas is not null, max_priority_fee_per_gas/ 1000000000, 0)) / sum(if(max_priority_fee_per_gas is not null, 1, 0)) maxPriorityFeePerGas, sum(fee) as evmFee, sum(burned_fee) as evmBurnedFee from substrate-etl.${relayChain}.evmtxs${paraID} where date(block_timestamp) = '${logDT}'`,
            "evmtransfers": `select count(*) as numEVMtransfer, sum(if(transfer_type = 'ERC20', 1, 0)) numERC20Transfers, sum(if(transfer_type = 'ERC721', 1, 0)) numERC721Transfers, sum(if(transfer_type = 'ERC1155', 1, 0)) numERC1155Transfers from substrate-etl.${relayChain}.evmtransfers${paraID} where date(block_timestamp) = '${logDT}'`
        };
        console.log(sqla);
        let r = {}
        let vals = [` loaded = 1 `];
