...
select token_address, account_address, sum(value) as value, sum(valuein) as receivedValue, sum(valueout) as sentValue, sum(valueCnt) as transferCnt , min(ts) as ts from transfersCombined group by token_address, account_address order by token_address`
 ...
 GROUP BY section, storage`
        ...
        async setupCallEvents() {
                ...
                // TODO: fetch from specVersions instead
                let tables = ["call", "evt"];
                for (const tbl of tables) {
                    let recs = null
                    if (tbl == "evt") {
                        recs = await this.execute_bqJob(`select id, para_id, relay_chain, extrinsic_id, extrinsic_hash, event_id, section, method, data_decoded, block_time from substrate-etl.polkadot_enterprise.events where
section in ("dexGeneral", "dex", "swaps", "xyk", "ammRoute", "amm", "curveAmm", "omnipool", "pablo", "stableAsset", "zenlinkProtocol") limit 500`);
                    } else {
                        recs = await this.execute_bqJob(`select id, para_id, relay_chain, extrinsic_id, extrinsic_hash, call_id, call_args, call_section, call_method, call_args_def, signer_ss58, signer_pub_key, block_time from substrate-etl.polkadot_enterprise.calls where
...
or ( call_method in ("buy", "sell") or call_method like 'swap%') ) limit 500`);
                    ...
                    for (const c of recs) {
                        ...
                        const tableId = `${tbl}_${section}_event_${event_method}`;
