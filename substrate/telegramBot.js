...
module.exports = class telegrambot extends PolkaholicDB {
    ...
    async initBot(){
        ...
        let bot = new TGBotAPI(token, {polling: true});
        this.bot = bot
    ...
    start(){
        ...
        const stakingInlineMenu = {
          reply_markup: JSON.stringify({
            inline_keyboard: [
              [{ text: 'Poolmember Dashboard 📈', callback_data: 'pool_member_daily_rewards' }, { text: 'Join Nomination Pool 👥', callback_data: 'join_pool' }],
              [{ text: 'Nominator Dashboard 💰', callback_data: 'nominator_daily_rewards' }, { text: 'Democroacy 🗳️', callback_data: 'democroacy' }],
              [{ text: 'Vew Account 👤', callback_data: 'show_account' }, { text: 'Staking Dashboard 📊', callback_data: 'staking_dashboard' }],
              [{ text: 'Set Polkadot Address ➕', callback_data: 'set_address' }, { text: 'Settings ⚙️', callback_data: 'settings' }, ]
            ]
          })
        };
        ...
        const setAddressMenu = {
          reply_markup: JSON.stringify({
            inline_keyboard: [
              [{ text: 'Set Polkadot Address ➕', callback_data: 'set_address' }]
            ]
          })
        };
        ...
        bot.on('message', async (msg) => {  // Notice the async keyword here
          ...
          if (this.userState[chatID] && this.userState[chatID].awaitingAddress) {
            ...
            try {
              ...
              bot.sendMessage(chatID, `Nicely Done! Your Polkadot address has been set to: [${address_ss58}](https://polkaholic.io/account/${address_ss58})`, { parse_mode: 'Markdown' });
        ...
        bot.on('callback_query', (callbackQuery) => {
          ...
          if (address_ss58 == undefined && category != "set_address"){
            ...
            this.userState[chatID] = { awaitingAddress: true };
            ...
            if (parse_mode != undefined){
                bot.sendMessage(chatID, responseText,  { parse_mode: parse_mode });
            }else{
