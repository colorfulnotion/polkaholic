...
order by msgHash`
    ...
    async xcm_reanalytics() {
        let sql = `select extrinsicHash, extrinsicID, xcmIndex, transferIndex, sourceTS, convert(xcmInfo using utf8) as xcmInfo from xcmtransfer where xcmInfo is not null and xcmInfoAudited = 1 order by extrinsicHash, xcmIndex, transferIndex limit 1000`
        try {
            console.log(sql);
            ...
            for (let i = 0; i < xcmmatches.length - 10; i++) {
                ...
                xcmInfo.push(x);
                let sql0 = `update xcmtransfer set xcmInfoAudited = 2, confidence = ${confidence}, amountReceived = ${amountReceived}, amountReceivedUSD = ${amountReceivedUSD} where extrinsicHash = '${extrinsicHash}' and xcmIndex = '${xcmIndex}' and transferIndex = '${transferIndex}'`
                if (confidence < .1) {
                    console.log(sql0);
                }
