...
module.exports = class Crawler extends Indexer {
    ...
    async crawlBackfill(chain, techniqueParams = ["mod", 0, 1]) {
        ...
        if (techniqueParams[0] == "mod") {
            let n = techniqueParams[1];
            let nmax = techniqueParams[2];
            sql = `select blockNumber, crawlBlock, 0 as crawlTrace, ${extraflds} attempted from block${chainID} where ( crawlBlock = 1 ${extracond} ) and blockNumber % ${nmax} = ${n} and blockNumber <= ${chain.blocksCovered} and attempted < 10 order by attempted, blockNumber desc limit 10000`
