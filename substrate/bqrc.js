    ...
    for (let s = 3; s <= 1; s++) {
        for (let test = 1; test <= 2; test++) {
            ...
            if (test == 1) {
                ...
                sql = "select symbol, sum(amount) amount, min(block_time) m0, max(block_time) m1 from substrate-etl.polkadot_enterprise.transfers where symbol = 'DOT' group by symbol order by amount desc limit 1";
            } else {
                ...
                sql = "select symbol, min(block_time) m0, max(block_time) m0, avg(price_usd) price_usd, sum(amount_usd) amount_usd_total from substrate-etl.polkadot_enterprise.transfers where symbol = 'DOT'  group by symbol order by amount_usd_total desc";
            }
            ...
            try {
                ...
                console.log(err);
                throw new Error(`An error has occurred.`, sql);
            }
        }
    }
    process.exit(0);
